...
describe('Environment Configuration', () => {
  ...
  describe('TRANSLATION_PROVIDER (HU-026)', () => {
    ...
    it('should accept "ollama" as TRANSLATION_PROVIDER', () => {
    ...
    it('should accept "libretranslate" as TRANSLATION_PROVIDER', () => {
      process.env['TRANSLATION_PROVIDER'] = 'libretranslate';

      const config = loadEnvConfig();

      expect(config.translation.provider).toBe('libretranslate');
    });

    it('should throw descriptive error for unknown TRANSLATION_PROVIDER value', () => {
      process.env['TRANSLATION_PROVIDER'] = 'deepl';

      expect(() => loadEnvConfig()).toThrow(
        'Invalid TRANSLATION_PROVIDER value: "deepl". Supported values are: "ollama", "libretranslate".',
      );
    });

    it('should throw error for empty TRANSLATION_PROVIDER', () => {
      process.env['TRANSLATION_PROVIDER'] = '';

      expect(() => loadEnvConfig()).toThrow('Invalid TRANSLATION_PROVIDER value');
    });

    it('should use default LIBRETRANSLATE_URL when not set', () => {
      delete process.env['LIBRETRANSLATE_URL'];

      const config = loadEnvConfig();

      expect(config.translation.libreTranslateUrl).toBe('http://libretranslate:5000');
    });

    it('should read LIBRETRANSLATE_URL from environment', () => {
      process.env['LIBRETRANSLATE_URL'] = 'http://my-libretranslate:5001';

      const config = loadEnvConfig();

      expect(config.translation.libreTranslateUrl).toBe('http://my-libretranslate:5001');
    });

    it('should use default LIBRETRANSLATE_TIMEOUT_MS when not set', () => {
      delete process.env['LIBRETRANSLATE_TIMEOUT_MS'];

      const config = loadEnvConfig();

      expect(config.translation.libreTranslateTimeoutMs).toBe(10000);
    });

    it('should read LIBRETRANSLATE_TIMEOUT_MS from environment', () => {
      process.env['LIBRETRANSLATE_TIMEOUT_MS'] = '20000';

      const config = loadEnvConfig();

      expect(config.translation.libreTranslateTimeoutMs).toBe(20000);
    });

    it('should throw error for invalid LIBRETRANSLATE_TIMEOUT_MS', () => {
      process.env['LIBRETRANSLATE_TIMEOUT_MS'] = 'bad-value';

      expect(() => loadEnvConfig()).toThrow(
        'Invalid integer value for LIBRETRANSLATE_TIMEOUT_MS',
      );
    });
  });
